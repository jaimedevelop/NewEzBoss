...
const EmployeeTimeCard: React.FC<{ portal: EmployeePortal; onChanged: (value: EmployeePortal) => void; refresh: (opts?: { background?: boolean }) => Promise<void> }> = ({ portal, onChanged, refresh }) => {
  ...
  const workedLabel = useMemo(() => {
    ...
    if (!isWorking) return hms((day.netWorkedMinutes ?? 0) * 60);
    const alreadyWorkedMs = (day.netWorkedMinutes ?? 0) * 60_000;
    const lastResumeAt = day.lunchEndedAt ?? day.breakEndedAt ?? day.clockInAt;
    const elapsedMs = Math.max(0, (now + offset) - new Date(lastResumeAt).getTime());
    const totalSeconds = Math.floor((alreadyWorkedMs + elapsedMs) / 1000);
  ...
  else if (activeBreak) main = <><p className="text-center font-semibold text-orange-700">Break · {clock}</p><button type="button" disabled={busy} onClick={() => void action('end-break')} className="primary w-full">End Break Early</button></>;
  else if (!day?.breakStartedAt) main = <button type="button" disabled={busy} onClick={() => void action('start-break')} className="primary w-full">Break</button>;
  else if (activeLunch) main = <><p className="text-center font-semibold text-orange-700">Lunch · {clock}</p><button type="button" disabled={busy} onClick={() => void action('end-lunch')} className="primary w-full">End Lunch Early</button></>;
  else if (!day?.lunchStartedAt) main = <button type="button" disabled={busy} onClick={() => void action('start-lunch')} className="primary w-full">Lunch</button>;
  else main = <p className="rounded-lg bg-green-50 p-3 text-center font-semibold text-green-800">Break and Lunch Complete</p>;
  return <section className="card space-y-4"><div><h2 className="font-bold">Today’s time</h2><p className="text-sm text-gray-600">Server time records your workday.</p></div>{error && <Alert>{error}</Alert>}{main}{clockedIn && !clockedOut && <button type="button" disabled={busy} onClick={() => void action('clock-out')} className="w-full rounded-lg border border-slate-400 px-4 py-3 font-semibold text-slate-800 disabled:opacity-50">Clock Out</button>}<dl className="grid grid-cols-2 gap-3 text-sm"><Metric label="Clock in" value={time(day?.clockInAt)} /><Metric label="Clock out" value={time(day?.clockOutAt)} /><Metric label="Break taken" value={duration(day?.actualBreakMinutes)} /><Metric label="Lunch taken" value={duration(day?.actualLunchMinutes)} /><Metric label="Gross" value={duration(day?.grossElapsedMinutes)} /><Metric label="Worked" value={workedLabel} /></dl></section>;
};
